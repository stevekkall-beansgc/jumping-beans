// Partner A — Coffee Co. Imperative WebMCP tool: get_matching_deals.
...
function normalizePreferencePlane(value) {
  ...
  if (!Object.keys(value).every((key) => ["feedStyle", "category", "maxPrice", "formats", "rules"].includes(key))) return null;
  const formats = Array.isArray(value.formats)
    ? value.formats.every((format) => ALLOWED_FORMATS.has(format)) && new Set(value.formats).size === value.formats.length ? [...value.formats] : null
    : [];
